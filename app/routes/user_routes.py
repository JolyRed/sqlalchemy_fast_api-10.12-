...
async def create_user(new_user: CreateUser, db: Annotated[Session, Depends(get_db)]):
    try:
        ...
        new_user_dict["slug"] = slugify(new_user_dict["username"]) # генерируем slug
...
async def update_user(user_id: int, updated_user: UpdateUser, db: Annotated[Session, Depends(get_db)]):
    ...
    if user is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User was not found")
    updated_user_dict = updated_user.dict(exclude={"id"}) # исключаем id из обновления
    updated_user_dict["slug"] = slugify(updated_user_dict["username"]) # генерируем slug
